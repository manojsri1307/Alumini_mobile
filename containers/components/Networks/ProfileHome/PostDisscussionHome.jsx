...
const PostDisscussionHome = () => {
  ...
  useEffect(() => {
    const getNews = async () => {
        try {
            const res = await axios.get(resources.APPLICATION_URL + `getDataBasedOnType?type=${encodeURIComponent('post discussion')}`)
            console.log("Post Disscussion Dataa", res)
  ...
  const renderNewsCard = (item) => (
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <View>
            <Text style={[styles.dateTime, {fontSize: 10, marginHorizontal: 'auto', marginTop: 6}]}>{item.postdiscussionCategory}</Text>
        </View>
        </View>
      </View>
      ...
      <View style={styles.cardContent}>
        ...
        {item.discussion && (
            <Text style={styles.dateTime}>{item.discussion}</Text>
        )}
...
export default PostDisscussionHome;